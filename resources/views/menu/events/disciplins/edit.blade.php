...
@section('title', 'Edit Disiplin - ' . $event->title)
@section('page-title')
Edit Disiplin <span class="bg-red-500 text-white text-sm px-2 py-1 rounded-full ml-2">{{ $event->title }}</span>
@endsection
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Disiplin</h2>
            <p class="text-gray-600 mt-1">Update discipline information</p>
        </div>
        <a href="{{ route('admin.events.disciplins.index', $event) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg flex items-center">
    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <form action="{{ route('admin.events.disciplins.update', [$event, $disciplin]) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text"
                    id="nama_disiplin"
                    name="nama_disiplin"
                    value="{{ old('nama_disiplin', $disciplin->nama_disiplin) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="sport_id" id="sport_id" required
                        ...
                        <option value="{{ $sport->id }}" {{ old('sport_id', $disciplin->sport_id) == $sport->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="venue_location_id" id="venue_location_id" required
                        ...
                        <option value="{{ $venue->id }}" {{ old('venue_location_id', $disciplin->venue_location_id) == $venue->id ? 'selected' : '' }}>
                            {{ $venue->nama }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="3" maxlength="1000"
                    ...
                    placeholder="Additional notes about this discipline">{{ old('keterangan', $disciplin->keterangan) }}</textarea>
                @error('keterangan')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-save mr-2"></i> Update Disiplin
                </button>
            </div>
        </form>
    </div>
