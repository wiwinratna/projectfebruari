...
@section('title', 'Tambah Kode Akomodasi - ' . $event->title)
@section('page-title')
Tambah Kode Akomodasi <span class="bg-red-500 text-white text-sm px-2 py-1 rounded-full ml-2">{{ $event->title }}</span>
@endsection
...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Create New Accommodation Code</h2>
                ...
                <div>
                    ...
                    <select id="icon_key" name="icon_key"
                        ...
                        @foreach(config('icon_catalog') as $key => $label)
                            <option value="{{ $key }}" @selected(old('icon_key') === $key)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="mt-3 text-sm text-gray-700 flex items-center gap-2">
                        ...
                        <span id="accom-preview" class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gray-50 border border-gray-200">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Preview logic (icon if chosen, fallback to code)
  ...
  function renderPreview(){
    ...
    fetch(url)
        ...
        if (canCode && !code) preview.appendChild(wrap(`<span class="text-gray-400">(no code yet)</span>`));
        })
        .catch(() => {
