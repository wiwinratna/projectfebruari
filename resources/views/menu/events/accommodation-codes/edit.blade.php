...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Accommodation Code</h2>
            <p class="text-gray-600 mt-1">Update accommodation code information</p>
        </div>
        <a href="{{ route('admin.master-data.accommodation-codes.index') }}"
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-800">Accommodation Code Information</h3>
        </div>

        <form action="{{ route('admin.master-data.accommodation-codes.update', $accommodationCode) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="kode" name="kode"
                        value="{{ old('kode', $accommodationCode->kode) }}"
                ...
                <div>
                    ...
                    <input type="text" id="keterangan" name="keterangan"
                        value="{{ old('keterangan', $accommodationCode->keterangan) }}" maxlength="1000"
                        class="w-full px-3 py-2.5 border border-gray-300 rounded-lg bg-white text-sm
                               focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent
                               @error('keterangan') border-red-500 @enderror"
                        placeholder="e.g., Standard hotel / Athlete village">
                ...
                <div>
                    ...
                    <select id="icon_key" name="icon_key"
                        ...
                        @foreach(config('icon_catalog') as $key => $label)
                            <option value="{{ $key }}"
                                @selected(old('icon_key', $accommodationCode->icon_key) === $key)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <div class="flex items-center gap-6">
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="show_icon" value="1"
                                ...
                                @checked(old('show_icon', $accommodationCode->show_icon ?? 1) == 1)>
                        ...
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="show_code" value="1"
                                class="rounded border-gray-300 text-red-500 focus:ring-red-500"
                                @checked(old('show_code', $accommodationCode->show_code ?? 1) == 1)>
...
<style>
  ...
  .ts-control .ts-placeholder { color: #9ca3af !important; }
  .ts-dropdown{
    border-radius: .5rem !important;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const preview    = document.getElementById('accom-preview');
  ...
  function renderPreview(){
    ...
    preview.innerHTML = '';
    const wrap = (html) => { const s=document.createElement('span'); s.innerHTML=html; return s; };
    ...
    fetch(url)
        ...
        preview.appendChild(wrap(
            `<span style="width:16px;height:16px;display:inline-flex;align-items:center;justify-content:center;">${svgHtml}</span>`
        ));
