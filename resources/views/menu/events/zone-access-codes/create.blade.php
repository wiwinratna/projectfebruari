...
@section('title', 'Tambah Kode Zona Akses - ' . $event->title)
@section('page-title')
Tambah Kode Zona <span class="bg-red-500 text-white text-sm px-2 py-1 rounded-full ml-2">{{ $event->title }}</span>
@endsection
...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Create New Kode Zona Akses</h2>
            <p class="text-gray-600 mt-1">Add a new zone access code for this event</p>
        </div>
        <a href="{{ route('admin.master-data.zone-access-codes.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg flex items-center">
    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <form action="{{ route('admin.master-data.zone-access-codes.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="kode_zona" name="kode_zona" value="{{ old('kode_zona') }}"
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                <a href="{{ route('admin.master-data.zone-access-codes.index') }}" class="px-6 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors duration-200">Cancel</a>
