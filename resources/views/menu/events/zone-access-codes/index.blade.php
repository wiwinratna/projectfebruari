...
@section('title', 'Kode Zona Akses - ' . $event->title)
@section('page-title')
Kode Zona Akses <span class="bg-red-500 text-white text-sm px-2 py-1 rounded-full ml-2">{{ $event->title }}</span>
@endsection
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-800">Semua Kode Zona Akses</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kode Zona</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Keterangan</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($zoneAccessCodes as $code)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $code->kode_zona }}</div>
                        </td>
                        <td class="px-6 py-4">
                    ...
                    <tr>
                        <td colspan="3" class="px-6 py-12 text-center">
                            <div class="text-gray-500">
                                ...
                                <p class="text-lg font-medium">No zone access codes found</p>
                                <p class="text-sm">Create your first zone access code to get started</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
<script>
    function deleteItem(id, name) {
        showConfirmModal('Delete Kode Zona Akses', 'Are you sure you want to delete this code?', `Kode: "${name}"`, () => performDelete(id));
    }
