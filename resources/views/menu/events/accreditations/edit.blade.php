...
@section('title', 'Edit Akreditasi - ' . $event->title)
@section('page-title')
Edit Akreditasi <span class="bg-red-500 text-white text-sm px-2 py-1 rounded-full ml-2">{{ $event->title }}</span>
@endsection
...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Akreditasi</h2>
            <p class="text-gray-600 mt-1">Update accreditation information</p>
        </div>
        <a href="{{ route('admin.master-data.accreditations.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg flex items-center">
    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <form action="{{ route('admin.master-data.accreditations.update', $accreditation) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="nama_akreditasi" name="nama_akreditasi" value="{{ old('nama_akreditasi', $accreditation->nama_akreditasi) }}"
                ...
                <div>
                    ...
                    <select name="jabatan_id" id="jabatan_id" required
                        ...
                        <option value="{{ $jabatan->id }}" {{ old('jabatan_id', $accreditation->jabatan_id) == $jabatan->id ? 'selected' : '' }}>{{ $jabatan->nama_jabatan }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="color" id="warna" name="warna" value="{{ old('warna', $accreditation->warna ?? '#3B82F6') }}"
                ...
                <div>
                    ...
                    <input type="text" id="keterangan" name="keterangan" value="{{ old('keterangan', $accreditation->keterangan) }}" maxlength="1000"
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                <a href="{{ route('admin.master-data.accreditations.index') }}" class="px-6 py-2 text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors duration-200">Cancel</a>
                <button type="submit" class="px-6 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors duration-200"><i class="fas fa-save mr-2"></i> Update Akreditasi</button>
            </div>
        </form>
    </div>
</div>
