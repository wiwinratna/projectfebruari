...
@section('title', 'Add Partner - NOCIS')
@section('page-title')
    Add Partner <span class="bg-purple-500 text-white text-sm px-2 py-1 rounded-full ml-2">Landing Page</span>
@endsection

@section('content')
<div class="max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('super-admin.partners.index') }}" class="text-gray-500 hover:text-gray-700 transition-colors">
            <i class="fas fa-arrow-left"></i>
        </a>
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Add New Partner</h2>
            <p class="text-gray-600 text-sm mt-1">This partner will appear in the Our Partners section of the landing page.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 space-y-5">
        <form method="POST" action="{{ route('super-admin.partners.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('name') border-red-500 @enderror"
                    placeholder="e.g. Sports Ministry of Indonesia">
                @error('name')
            ...
            <div>
                ...
                <input type="url" name="website" id="website" value="{{ old('website') }}"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 @error('website') border-red-500 @enderror"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="2" maxlength="500"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 @error('description') border-red-500 @enderror"
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit"
                    class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-lg font-semibold text-sm transition-colors">
