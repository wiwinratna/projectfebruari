...
@section('title', 'Edit Partner - NOCIS')
@section('page-title')
    Edit Partner <span class="bg-purple-500 text-white text-sm px-2 py-1 rounded-full ml-2">Landing Page</span>
@endsection
...
<div class="max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            ...
            <p class="text-gray-600 text-sm mt-1">Update {{ $partner->name }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 space-y-5">
        <form method="POST" action="{{ route('super-admin.partners.update', $partner) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $partner->name) }}" required
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 @error('name') border-red-500 @enderror">
                @error('name')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="2" maxlength="500"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 @error('description') border-red-500 @enderror">{{ old('description', $partner->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
